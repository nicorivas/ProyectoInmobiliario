...
class DateTimeEncoder(json.JSONEncoder):
    #Special class for serializaing datetime to json
    def default(self, o):
            ...
            if cv == term:
                ...
                for i in range(50):
                    ...
                    if not isinstance(element, (int, float, str)):
                        ...
                        if len(elements) == 0:
                            elements[term+'_'+ currency +'_valor_1'] = element
                        elif len(elements) == 1:
                            elements[term+'_'+ currency +'_valor_2'] = element
                            return elements
                        else:
                            return elements


#Fields for seach and seacrh parameters
...
file_list_= pd.read_excel('D:/TASACIONES GENERALES CHILE/lista_tasaciones.xlsx', encoding = 'utf8')
file_list = file_list_.to_dict(orient='records')

for j, i in enumerate(file_list):
    if i['extencion'] in file_extensions and not i['extracted'] and i['banco']==bank:
        app_meta = i
        print(i)
        file = i['dir']
        print(file)
        ...
        json_file = open( 'D:/TASACIONES GENERALES CHILE/extracted files/'+ name +'.json', 'w')
        json_file.write(json.dumps(appraisal, cls=DateTimeEncoder))
        json_file.close()
        #file_list_.loc['extracted'][j]=True
        #file_list_.to_excel('D:/TASACIONES GENERALES CHILE/lista_tasaciones.xlsx')
print(appraisal)