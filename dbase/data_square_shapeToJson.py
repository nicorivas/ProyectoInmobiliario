...
def with_ogr2ogr(source='INE2016',do_simplify=1,do_convert=1):
    ...
    if source == 'INE2016':
        ...
        filepath_in = sorted(glob.glob(base_dir+
            '/sources/INE_CartografiaPrecenso2016/R*/Manzana_Precensal.shp'))
        filepath_out = [base_dir+
            '/manzanas/shp/manzanas_'+slugify(code_to_region_name[i])+'.shp'
            for i in code_to_region_name.keys()]

    if do_simplify:
        simplify(filepath_in,filepath_out,ref=0.00001)

    if source == 'INE2016':
        filepath_in = ['../data/geo/chile/manzanas/shp/manzanas_'+
            slugify(code_to_region_name[i+1])+'.shp'
            for i in range(len(code_to_region_name))]
        filepath_out = ['../data/geo/chile/manzanas/json/manzanas_'+
