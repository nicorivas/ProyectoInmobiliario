#!/usr/bin/env python
'''
Script to check integrity and completness of realestate source data files
'''
import datetime
from tools import *
from globals import *
...
for region in regions:
    basepath = datapath+'/source/{}/'.format(slugify(region.name))
    if not os.path.isdir(basepath):
        warning('{} DIRECTORY DOES NOT EXIST'.format(region.name),log)
    else:
        say('{} DIR OK'.format(region.name))
    ...
    for commune in communes_region:
        ...
        if not os.path.isdir(basepath):
            error('{} DIRECTORY NOT FOUND'.format(commune.name),log)
        else:
            say('{} DIR OK'.format(commune.name))
        ...
        for source in sources:
            ...
            if not os.path.isdir(basepath):
                error('{} {} DIRECTORY NOT FOUND'.format(commune.name,source),log)
                continue

            say('{} {} OK'.format(commune.name,source))
            ...
            if not os.path.isfile(pfile):
                ...
                if not os.path.isfile(tmp):
                    error('{} {} URLS FILE NOT FOUND'.format(commune.name,source),log)
                    continue
                else:
                    warning('{} {} URLS FILE FALTA TILDE?'.format(commune.name,source),log)
                    pfile = tmp

            data = json.load(open(pfile))
            say('{} {} URLS: {}'.format(commune.name,source,len(data)))
            ...
            if not os.path.isfile(pfile):
                ...
                if not os.path.isfile(tmp):
                    error('{} {} PROPERTIES FILE DOES NOT EXIST'.format(commune.name,source),log)
                    continue
                else:
                    warning('{} {} PROPERTIES FILE FALTA TILDE?'.format(commune.name,source),log)
                    pfile = tmp

            data = json.load(open(pfile))
            say('{} {} PROPERTIES FILE PROPERTIES: {}'.format(commune.name,source,len(data)))

            dirs = [a for a in glob.glob(basepath + '/*') if os.path.isdir(a)]
            if len(dirs) == 0:
                warning('{} {} NO DATA DIRECTORY'.format(commune.name,source),log)
            for d in dirs:
                date = datetime.datetime.strptime(d[d.rfind('/')+1:],"%Y-%m-%dT%H-%M-%S")
                say('{} {} {} OK'.format(commune.name,source,date))
                ...
                for file_suffix in file_suffixs:
                    ...
                    if not os.path.isfile(filename):
                        ...
                        if not os.path.isfile(tmp):
                            warning('{} {} {} {} NOT FOUND'.format(commune.name,source,date,file_suffix),log)
                            continue
                        else:
                            warning('{} {} {} {} FALTA TILDE?'.format(commune.name,source,date,file_suffix),log)
                            filename = tmp

                    data = json.load(codecs.open(filename, 'r', 'utf-8-sig'))
                    say('{} {} {} {} # = {}'.format(commune.name,source,date,file_suffix,len(data)))
